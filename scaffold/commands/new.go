...
import (
	...
	"github.com/Summer2021-Project/scaffold/logic"
...
func (t *NewCommand) NewProject(name, selectType, selectEnv, selectConf, selectLog, selectDb, selectRedis string) {
	...
	srcDir := fmt.Sprintf("%s/pkg/mod/github.com/Summer2021-Project/%s-skeleton@%s", goPath, selectType, ver)
	if _, err := os.Stat(srcDir); err != nil {
		cmd := exec.Command("go", "get", fmt.Sprintf("github.com/Summer2021-Project/%s-skeleton@%s", selectType, ver))
		fmt.Printf("Skeleton local not found, exec 'go get github.com/Summer2021-Project/%s-skeleton@%s'\n", selectType, ver)
		total := 0
		switch selectType {
		case CLI:
			total = 7695
		...
		go func() {
			path := fmt.Sprintf("%s/pkg/mod/cache/download/github.com/Summer2021-Project/%s-skeleton/@v/%s.zip", goPath, selectType, ver)
			for {
				f, err := os.Open(path)
				if err != nil {
					continue
		...
		fmt.Println(fmt.Sprintf("Skeleton 'github.com/Summer2021-Project/%s-skeleton@%s' download is completed", selectType, ver))
	} else {
		fmt.Println(fmt.Sprintf("Skeleton 'github.com/Summer2021-Project/%s-skeleton@%s' local found", selectType, ver))
	}
	...
	if selectEnv == None {
		if err := logic.ReplaceMain(dest, fmt.Sprintf(`_ "github.com/Summer2021-Project/%s-skeleton/config/dotenv"`, selectType), ""); err != nil {
	...
	switch selectConf {
	case Viper:
		if err := logic.ReplaceMain(dest, fmt.Sprintf(`_ "github.com/Summer2021-Project/%s-skeleton/config/configor"`, selectType), ""); err != nil {
			panic(errors.New("Replace failed"))
		}
		_ = os.RemoveAll(fmt.Sprintf("%s/config/configor", dest))
		break
	case Configor:
		if err := logic.ReplaceMain(dest, fmt.Sprintf(`_ "github.com/Summer2021-Project/%s-skeleton/config/viper"`, selectType), ""); err != nil {
			panic(errors.New("Replace failed"))
		}
	...
	if selectLog == None && selectDb == None && selectRedis == None {
		if err := logic.ReplaceMain(dest, fmt.Sprintf(`_ "github.com/Summer2021-Project/%s-skeleton/di"`, selectType), ""); err != nil {
			panic(errors.New("Replace failed"))
		}
	}

	fmt.Print(" - Processing package name")
	if err := logic.ReplaceAll(dest, fmt.Sprintf("github.com/Summer2021-Project/%s-skeleton", selectType), name); err != nil {
		panic(errors.New("Replace failed"))
	}
