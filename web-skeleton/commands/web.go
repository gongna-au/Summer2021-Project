...
import (
	...
	"github.com/Summer2021-Project/web-skeleton/routes"
...
func (t *WebCommand) Main() {
	...
	// templates
	router.LoadHTMLGlob(fmt.Sprintf("%s/../templates/*", cli.App().BasePath))

	// static file
	router.Static("/static", fmt.Sprintf("%s/../public/static", cli.App().BasePath))
	router.StaticFile("/favicon.ico", fmt.Sprintf("%s/../public/favicon.ico", cli.App().BasePath))
