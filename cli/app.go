...
import (
	...
	"github.com/Summer2021-Project/cli/errno"
...
func (t *application) Run() {
	defer func() {
		if err := recover(); err != nil {
			switch err.(type) {
			case *errno.NotFoundError, *errno.UnsupportedError:
				fmt.Println(err)
				return
			default:
				panic(err)
	...
	if command == "" {
		...
		p := argv.Program().Path
		panic(errno.NewNotFoundError(fmt.Errorf("flag provided but not defined: '%s', see '%s --help'.", f, p)))
...
func (t *application) call() {
	...
	if cmd == nil {
		panic(errno.NewNotFoundError(fmt.Errorf("'%s' is not command, see '%s --help'.", command, argv.Program().Path)))
	}
...
func (t *application) validateOptions() {
	...
	for f := range flag.Options().Map() {
		if !inArray(f, flags) {
			...
			panic(errno.NewNotFoundError(fmt.Errorf("flag provided but not defined: '%s', see '%s%s --help'.", f, p, c)))
		}
	}
}
...
func (t *application) globalHelp() {
	...
	fmt.Println("")
	fmt.Println("Developed with Dubbo-go framework.")
}
